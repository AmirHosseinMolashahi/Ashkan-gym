...
import DiscountCardsSkeleton from './discountCardSkeleton/DiscountCardSkeleton';

const DiscountCard = ({data, editDiscount, deleteDiscount, loading}) => {
  return (
    <div className={styles.discountCard}>
      {loading === true ? (
        <DiscountCardsSkeleton />
      ) : (
        data.length > 0 ? (
          data.map((item, index) => (
